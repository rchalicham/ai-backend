...
from .models import ConstraintDiagnostics


class ReceiptConstraintSerializer:
    def to_dict(self, value: Any) -> dict[str, Any]:
        payload = self._value(value)
        if not isinstance(payload, dict):
            raise TypeError("receipt_constraint_serialization_requires_object")
        return payload
    ...
    def _value(self, value: Any) -> Any:
        ...
        if isinstance(value, Enum):
            return value.value
        if isinstance(value, ConstraintDiagnostics):
            return {
                "valid": value.valid,
