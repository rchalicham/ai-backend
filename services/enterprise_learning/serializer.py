...
class EnterpriseLearningSerializer:
    def to_dict(self, value: Any) -> dict[str, Any]:
        result = self._value(value)
        if not isinstance(result, dict):
            raise TypeError("enterprise_learning_serialization_requires_object")
        return result
    ...
    def _value(self, value: Any) -> Any:
        ...
        if is_dataclass(value):
            result = {field.name: self._value(getattr(value, field.name)) for field in fields(value)}
            if value.__class__.__name__ == "LearningDiagnostics":
