...
class HouseholdIntelligenceSerializer:
    def to_dict(self, value):
        result = self._value(value)
        if not isinstance(result, dict):
            raise TypeError("household_serialization_requires_object")
        return result

    def to_json(self, value, *, pretty=False):
    ...
    def _value(self, value):
        ...
        if is_dataclass(value):
            result = {x.name: self._value(getattr(value, x.name)) for x in fields(value)}
            if value.__class__.__name__ == "HouseholdDiagnostics":
        ...
        if isinstance(value, tuple):
            ...
            return [self._value(x) for x in value]
